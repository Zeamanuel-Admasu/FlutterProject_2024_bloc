    ...
    if (id) {
        fetch("/reserve/userreservations", {
            ...
            .then(data => {
                ...
                let updateButtons = Array.from(document.getElementsByClassName("updateButton"));
                ...
                updateButtons.forEach(<HTMLInputElement>(btn) => {
        ...
        function createHTMLTable(arr) {
            ...
            arr.forEach((elt: { tableNumber: string; Type: string; time: string; date: string; Number_of_people: string; }) => {
                ...
                let updbutton:HTMLButtonElement = document.createElement("button");
                button.classList.add("deleteButton");
                updbutton.classList.add("updateButton");
                let content6 = document.createTextNode("Cancel");
                let content7 = document.createTextNode("Update");
                button.setAttribute("value", elt.tableNumber);
                button.setAttribute("sometime", elt.time);
                button.setAttribute("type", "submit");
                updbutton.setAttribute("value", elt.tableNumber);
                updbutton.setAttribute("sometime", elt.time);
                updbutton.setAttribute("type", "submit");
                button.appendChild(content6);
                updbutton.appendChild(content7);
